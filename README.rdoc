...
Lower level examples, if your url is already broken down into pieces:
  ...
  response_object = HttpSession.use(host_string).request # defaults are: false, 80, and '/', {}, :get, {}
  response_object = HttpSession.use(host_string, use_ssl_boolean, port_integer).request(uri_string, headers_hash, get_or_post_symbol, post_params_hash)

All the above return:
* a <tt>Net::HTTPResponse</tt> object in all its glory

Or raise any of the following:
* <tt>Timeout::Error</tt> or <tt>Errno::*</tt> (<tt>SystemCallError</tt> subclass) or <tt>OpenSSL::SSL::SSLError</tt> - for various connection-related issues
